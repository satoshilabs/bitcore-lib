...
var address = new btc.PrivateKey(bn).toAddress();
```
...
var address = new btc.PrivateKey(wif).toAddress();
```

## Create a Transaction
```javascript
var privateKey = new btc.PrivateKey('L1uyy5qTuGrVXrmrsvHWHgVzW9kKdrp27wBC7Vs6nZDTF2BRUVwy');
var utxo = {
...
var transaction = new btc.Transaction()
    .from(utxo)
    .addData('btc rocks') // Add OP_RETURN data
    .sign(privateKey);
```
...
var address = new btc.Address(publicKeys, requiredSignatures);
```

## Spend from a 2-of-2 multisig P2SH address
```javascript
var privateKeys = [
  new btc.PrivateKey('91avARGdfge8E4tZfYLoxeJ5sGBdNJQH4kvjJoQFacbgwmaKkrx'),
  new btc.PrivateKey('91avARGdfge8E4tZfYLoxeJ5sGBdNJQH4kvjJoQFacbgww7vXtT')
];
var publicKeys = privateKeys.map(btc.PublicKey);
var address = new btc.Address(publicKeys, 2); // 2 of 2

var utxo = {
  "txId" : "153068cdd81b73ec9d8dcce27f2c77ddda12dee3db424bff5cafdbe9f01c1756",
  "outputIndex" : 0,
  "address" : address.toString(),
  "script" : new btc.Script(address).toHex(),
  "satoshis" : 20000
};
