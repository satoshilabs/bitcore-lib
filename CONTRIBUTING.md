Contributing to BtcLib
=======

We're working hard to make *btc* the most powerful JavaScript library for working with bitcoin. Our goal is to have *btc* be a library that can be used by anyone interested in bitcoin, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://github.com/owstack/btc-lib/issues (Development Issues)
...
These are some global design goals in btc that any change must adhere.
...
When possible, btc objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of btc methods. For example, calling `JSON.stringify` on an btc object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'btc-lib' and Look up Classes from There
...
```sh
git remote add owstack git@github.com:owstack/btc-lib.git
...
```
Finally go to [github.com/owstack/btc-lib](https://github.com/owstack/btc-lib) in your web browser and issue a new pull request.
...
[github.com/owstack/owstack/btc-lib/issues](https://github.com/owstack/owstack/btc-lib/issues).
